...
use crate::{
    ...
    qc, Error, Field, QuadScaled, Scaled, Value, Var,
};
...
#[derive(Clone)]
pub struct CrtGadget<N: Field> {
    pub(crate) rns: Rns<N>,
    pub(crate) wrong_mod: Big<N>,
}

impl<N: Field> CrtGadget<N> {
    pub fn new(limb_size: usize, sublimb_size: usize, wrong_mod: &BigUint) -> Self {
        let mod_size = wrong_mod.bits() as usize;
        let rns = Rns::construct(mod_size, limb_size, sublimb_size);
        let wrong_mod = rns.big_from_uint(wrong_mod);
        Self { rns, wrong_mod }
    }
    ...
    pub(crate) fn witgen_red(&self, w: Value<&BigInt>) -> DivModResult {
        ...
        self.rns.witgen_red(w, wrong_mod.into())
    }
    ...
    pub(crate) fn witgen_mul(
        ...
        let wrong_mod = self.wrong_mod.int();
        self.rns.witgen_mul(w0, w1, d, to_add, wrong_mod.into())
    }
    ...
    pub fn reduce(&self, ac: &mut AbstractCircuit<N>, w: &VarBig<N>) -> VarBig<N> {
        ...
        let quot = witness.q().map(|q| {
            ac.log_max("big-field-q-max", q.bits());
            N::from_int(q).unwrap()
        });
        let quot = ac
    ...
    pub fn assert_zero(&self, ac: &mut AbstractCircuit<N>, w: &VarBig<N>) -> Result<(), Error> {
        ...
        let quot = ac.decompose_assign_signed(self.rns.sublimb_size, quot_size as usize, &quot)?;
    ...
    pub fn mul(
        ...
        // result is a term to subtract
        to_sub.push(res.clone());

        // mul into combination terms
        let quad = w0.product(w1);
        let mut lin = quot.product_constant(&self.wrong_mod);

        // negate p * q
        lin.iter_mut()
            .for_each(|row| row.iter_mut().for_each(Scaled::neg_assign));
    ...
    pub fn mul_constant(
        ...
        // convert into cobination terms
        let mut lin = w0.product_constant(constant);
        lin.iter_mut()
            .zip(quot.product_constant(&self.wrong_mod))
            .for_each(|(ww, mut pq)| {
                pq.iter_mut().for_each(Scaled::neg_assign);
                ww.extend(pq);
            });
        ...
        // eval carries of partial schoolbook multiplication
        self.rns.eval_quad(ac, &[], &lin, max_carries);

        let mut qc = qc!();
        qc += w0.nat() * constant.nat();
        qc += to_add.iter().map(VarBig::nat);
        qc -= to_sub.iter().map(VarBig::nat);
        qc -= quot.nat() * self.wrong_mod.nat();
        ac.zero_sum(qc).unwrap();
    ...
    pub fn mul_div(
        ...
        let mut lin = quot
            .product_constant(&self.wrong_mod)
            .iter()
            .map(|row| row.iter().map(std::ops::Neg::neg).collect_vec())
            .collect_vec();
    ...
    pub fn assert_in_field(
        ...
        let minus_one = self.wrong_mod().uint() - 1usize;
        let minus_one = self.rns.big_from_uint(&minus_one);
        ...
        let mut qc = qc!(minus_one.limb_at(0).nat());
        qc -= w0.limb_at(0);
        qc -= result[0].var();
        ...
        for i in 1..self.rns.n_limbs - 1 {
            let mut qc = qc!(minus_one.limb_at(i).nat());
            qc -= w0.limb_at(i);
            qc -= result[i].var();
    ...
    pub fn neg(&self, ac: &mut AbstractCircuit<N>, w: &VarBig<N>) -> VarBig<N> {
        w.sub_from_constant(ac, &self.wrong_mod)
    }
    ...
    pub fn select_from_table(
        ...
        self.rns.select_from_table(ac, selector, table)
    }
    ...
    pub fn select_from_constant_table(
        ...
        self.rns.select_from_constant_table(ac, selector, table)
    }
    ...
    pub fn write(
        ...
        self.rns.write(ac, segment, address, e)
    }
    ...
    pub fn read(
        ...
        self.rns.read(ac, segment, address_offset, address_fine)
    }
    ...
    pub fn write_constant(
        ...
        self.rns.write_constant(ac, segment, address, e)
    }
    ...
    pub fn read_constant(
        ...
        self.rns
            .read_constant(ac, segment, address_offset, address_fine)
    }
}
